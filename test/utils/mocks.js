...
reqs['https://www.googleapis.com/youtube/v3/videos'] = googleapis;
...
sinon.stub(global.jQuery, 'get').callsFake((path, query) => {
  if (path === 'https://www.googleapis.com/youtube/v3/videos') {
    return googleapis();
  }
...
sinon.stub(global.jQuery, 'post').callsFake((path) => {
  if (ajaxSuccess) {
...
// for now only used to load youtube inframe api
