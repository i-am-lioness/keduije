...
/* Middleware */
  ...
  app.get('/api/media', (req, res, next) => {
    db.collection('media')
    .findOne({ changeset: ObjectId(req.query.changesetID) })
    .then((media) => {
      res.send(media);
    })
    .catch(next);
...
*/
  ...
  app.get('/api/revisions', ensureLoggedIn(), (req, res, next) => {
    db.collection('revisions').find({ changeset: ObjectId(req.query.changesetID), state: 'done' })
    .toArray()
    .then((revisions) => {
      res.send(revisions);
    })
  ...
  app.get('/api/myLines', ensureLoggedIn(), (req, res, next) => {
    db.collection('lines').find({ changeset: ObjectId(req.query.changesetID) })
    .toArray()
    .then((lines) => {
      res.send(lines);
    })
  ...
  app.post('/api/media/:mediaID/addline', ensureLoggedIn(), (req, res, next) => {
    ...
    validate(req.body).then((newLine) => {
      newLine.creator = req.user._id;
      newLine.media = ObjectId(req.params.mediaID);
      newLine.changeset = ObjectId(req.params.changesetID);
      newLine.version = 1;
