...
function sendDataToTemplate(req, res, next) {
  res.locals.user = req.user;
  res.locals.title = 'Kezie';
  next();

  users.log(req).then(mail.send);
}
...
/* Middleware */
  ...
  app.use(users.initialize(db));
  app.use(users.session());
  app.use(sendDataToTemplate);
  ...
  app.post('/api/logError', (req, res) => {
    ...
    mail.send(JSON.stringify(error), 'client error');
  });
  ...
  app.post('/api/media/new', ensureLoggedIn(), requireRole('admin'), (req, res, next) => {
    ...
    db.collection('media').insertOne(newMedia).then((result) => {
      res.append('Inserted-Id', result.insertedId);
      res.send(newMedia.slug);
      // TO DO: test
      db.collection('changesets').updateOne({ _id: newMedia.changeset }, { $set: { media: newMedia._id } });
    })
    .catch(next);
  });
...
*/
  ...
  app.get('/api/media/:mediaID', (req, res, next) => {
    db.collection('media').findOne({ _id: ObjectId(req.params.mediaID) })
      .then((media) => {
        res.send(media);
      })
      .catch(next);
    ...
    db.collection('media').findOne({ _id: ObjectId(req.params.mediaID) })
      .then((media) => {
        if (media.pendingRollbacks && (media.pendingRollbacks.length > 0)) {
          ...
          db.collection('changesets').insertOne({
            type: 'edit',
            user: req.user._id,
  ...
  app.post('/api/lines/edit/:forID', ensureLoggedIn(), (req, res, next) => {
    ...
    r.execute(revisionTypes.LINE_EDIT, req)
      .then((line) => {
  ...
  app.post('/api/media/:mediaID/addline', ensureLoggedIn(), (req, res, next) => {
    const r = new Revision(db);
    r.execute(revisionTypes.LINE_ADD, req)
      .then((result) => {
  ...
  app.post('/api/media/edit/:forID', (req, res, next) => {
    ...
    const r = new Revision(db);
    r.execute(revisionTypes.INFO_EDIT, req)
      .then((media) => {
        res.send(media);
      })
      .catch(next);
...
function onDbConnectFail(err) {
  console.error(`Failed to connect to DB at URL:${DB_URL}`);
  console.error(err);
  throw err;
  // to do: retry?
}
...
function run() {
  return MongoClient.connect(DB_URL).then(startServer, onDbConnectFail);
}
