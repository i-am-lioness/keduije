...
function startServer(db, env, start) {
  ...
  app.get('/login',
    (req, res) => {
      req.session.returnTo = req.session.returnTo || req.header('Referer');
      res.send('<form action="/login" method="post"><input type="submit" value="admin login"/><input type="hidden" name="username" value="a"/><input type="hidden" name="password" value="a"/></form>');
    }
  );

  app.post('/login', users.authenticate('local'));
