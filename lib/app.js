...
*/
  ...
  app.post('/api/media/:mediaID/addline', ensureLoggedIn(), (req, res, next) => {
    ...
    validate(req.body).then(() => {
      ...
      return db.collection('lines').insertOne(req.body);
    }).then(() => {
      sendLines(req.params.mediaID, res, next);
    }).catch(next);
  });
...
function onDbConnectFail(err) {
  ...
  console.error(err);
  // to do: retry?
  // to do: consider throwing error, so that Heroku automatically retries?
}

function run() {
  return MongoClient.connect(DB_URL).then(startServer).catch(onDbConnectFail);
}
